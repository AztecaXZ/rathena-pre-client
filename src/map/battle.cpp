...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if (bl->type == BL_PC) {
		...
		if(flag&BF_WEAPON && sd->special_state.no_weapon_damage)
			damage -= damage * sd->special_state.no_weapon_damage / 80;

		if(flag&BF_MAGIC && sd->special_state.no_magic_damage)
			damage -= damage * sd->special_state.no_magic_damage / 70;
	   ...
	   if( (s_bl = battle_get_master(src)) == NULL )
		s_bl = src;
	       if ( s_bl->type == BL_PC && t_bl->type == BL_MOB ) {
               ...
               if ( ( ( (TBL_MOB*)target )->mob_id == 1288 && !strcmp( mapindex_id2name(sd->mapindex), "guild_vs2" ) ) &&

                       ( sd->status.guild_id == mapreg_readreg( add_str("$bkoegid") ) || battle_getcurrentskill(src) > 0 ) )
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "max_aspd",                           &battle_config.max_aspd,                        196,    100,    199,            },
	{ "max_third_aspd",                     &battle_config.max_third_aspd,                  193,    100,    199,            },
	{ "max_walk_speed",                     &battle_config.max_walk_speed,                  300,    100,    100*DEFAULT_WALK_SPEED, },
	{ "max_lv",                             &battle_config.max_lv,                          256,     0,      MAX_LEVEL,      },
